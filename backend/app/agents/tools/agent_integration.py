...
def run_codex_agent(ctx: RunContext[Deps], prompt: str) -> str:
    ...
    with logfire.span("run_codex_agent", prompt=prompt) as span:
        ...
        if os.environ.get("MOCK_AGENT_CLI") == "true":
            logger.info(
                f"MOCK_AGENT_CLI is enabled. Returning mock response for Codex prompt: {prompt[:50]}..."
            )
        ...
        try:
            ...
            if result.returncode != 0:
                error_msg = (
                    f"Codex agent failed with code {result.returncode}.\nStderr: {result.stderr}"
                )
            ...
            return f"Error executing Codex agent: {e!s}"
...
def run_claude_agent(ctx: RunContext[Deps], prompt: str) -> str:
    ...
    with logfire.span("run_claude_agent", prompt=prompt) as span:
        ...
        if os.environ.get("MOCK_AGENT_CLI") == "true":
            logger.info(
                f"MOCK_AGENT_CLI is enabled. Returning mock response for Claude prompt: {prompt[:50]}..."
            )
            span.set_attribute("mock", True)
        ...
        try:
            ...
            if result.returncode != 0:
                error_msg = (
                    f"Claude agent failed with code {result.returncode}.\nStderr: {result.stderr}"
                )
                logger.error(error_msg)
                span.record_exception(Exception(error_msg))
                return error_msg
            ...
            span.record_exception(e)
            return f"Error executing Claude agent: {e!s}"

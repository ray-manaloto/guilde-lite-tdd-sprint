...
Each generated project includes a CLI tool named `guilde_lite_tdd_sprint`. Run commands from the `backend/` directory:
...
uv run guilde_lite_tdd_sprint db downgrade            # Rollback migration
# User commands
uv run guilde_lite_tdd_sprint user create-admin       # Create admin user
uv run guilde_lite_tdd_sprint user create             # Create regular user
uv run guilde_lite_tdd_sprint user list               # List all users
```
...
```yaml
scrape_configs:
  - job_name: 'guilde_lite_tdd_sprint'
    static_configs:
...
3. **Build and push Docker image**:
   ```bash
   docker build -t your-registry/guilde_lite_tdd_sprint:latest ./backend
   docker push your-registry/guilde_lite_tdd_sprint:latest
   ```

4. **Update image** in `kubernetes/kustomization.yaml`:
   ```yaml
   images:
     - name: guilde_lite_tdd_sprint
       newName: your-registry/guilde_lite_tdd_sprint
       newTag: latest
   ```
...
kubectl scale deployment guilde_lite_tdd_sprint-backend -n guilde_lite_tdd_sprint --replicas=3

# View pods
kubectl get pods -n guilde_lite_tdd_sprint

# View logs
kubectl logs -f deployment/guilde_lite_tdd_sprint-backend -n guilde_lite_tdd_sprint
```
...
# Server
uv run guilde_lite_tdd_sprint server run --reload
uv run guilde_lite_tdd_sprint server routes

# Database (Alembic wrapper)
uv run guilde_lite_tdd_sprint db init
uv run guilde_lite_tdd_sprint db migrate -m "Add users"
uv run guilde_lite_tdd_sprint db upgrade

# Users
uv run guilde_lite_tdd_sprint user create-admin       # Create admin (interactive)
uv run guilde_lite_tdd_sprint user create             # Create user (interactive)
uv run guilde_lite_tdd_sprint user list               # List all users
```
...
```bash
uv run guilde_lite_tdd_sprint cmd seed --count 100
uv run guilde_lite_tdd_sprint cmd seed --dry-run
```
...
| Task | Command |
|------|---------|
| Create migration | `uv run --directory backend guilde_lite_tdd_sprint db migrate -m "message"` |
| Apply migrations | `uv run --directory backend guilde_lite_tdd_sprint db upgrade` |
| Rollback migration | `uv run --directory backend guilde_lite_tdd_sprint db downgrade` |
| Show current | `uv run --directory backend guilde_lite_tdd_sprint db current` |
| Show history | `uv run --directory backend guilde_lite_tdd_sprint db history` |

### Users

| Task | Command |
|------|---------|
| Create admin | `uv run --directory backend guilde_lite_tdd_sprint user create-admin` |
| Create user | `uv run --directory backend guilde_lite_tdd_sprint user create` |
| List users | `uv run --directory backend guilde_lite_tdd_sprint user list` |

### Celery

| Task | Command |
|------|---------|
| Start worker | `uv run --directory backend guilde_lite_tdd_sprint celery worker` |
| Start beat | `uv run --directory backend guilde_lite_tdd_sprint celery beat` |
| Start flower | `uv run --directory backend guilde_lite_tdd_sprint celery flower` |
